...
export const TeamDrawerSquad = ():JSX.Element => {
    const draftedSquadPlayers = useAppSelector((state) => state.draft.players.squad);
    return (
        <>
            {['goalkeepers', 'defenders', 'midfielders', 'attackers'].map((position) => (
            <ul role="list" className="divide-y divide-gray-200">
                {draftedSquadPlayers[position].filter(player => !player.isPlaceholder).map((player, index) => (                                            
                    <li key={player.web_name}>
                        <div className="group relative flex items-center px-5 py-6">
                            ...
                            <div className='flex flex-col sm:flex-row justify-center items-center content-center gap-0'>
                                <PlayerRemover index={index} elementType={player.element_type} isBench={false}/>
                                <TeamDrawerMenu />
                            </div>
                        </div>
                    </li>
